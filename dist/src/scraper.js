...
exports.KNOWN_CODES = KNOWN_CODES;
function getCityCode(city) {
    const n = norm(city);
    if (KNOWN_CODES[n])
        return KNOWN_CODES[n];
...
async function applyNonStopFilter(page) {
    ...
    try {
        ...
        console.warn("⚠️  Filtre hatası:", err.message);
    }
}
// ─── Uçuşları çek ────────────────────────────────────────────────────────────
async function scrapeFlights(page) {
    ...
    return await page.evaluate(() => {
        const cards = Array.from(document.querySelectorAll("li.item.journey, li[class*='journey'][class*='available']"));
...
async function discoverCityCode(page, cityName, field) {
    ...
    try {
        ...
        const picked = await page.evaluate((city) => {
        ...
        if (code)
            console.log(`  🔑 ${cityName} kodu bulundu: ${code}`);
        return code;
    }
    catch (e) {
        return null;
...
async function fillCityByForm(page, origin, destination) {
    let originOk = false;
    let destOk = false;
    // Kalkış
    try {
        ...
        console.warn("  ⚠️  Kalkış doldurulamadı:", e.message);
    }
    // Varış
    try {
        ...
        console.warn("  ⚠️  Varış doldurulamadı:", e.message);
    }
